...
void MenuState::render() {
   ...
   DrawRectangleV({0.0f, 0.0f}, {600.0f * cr, GetScreenHeight() * 1.0f}, Fade(RED, 0.5f));
   drawTextSemiCentered(font, {cr * 25.0f, cr * 100.0f}, "      ABYSS\n ASCENDANT 3", 80.0f, WHITE);
   titleButtons.render();

   if (titleButtons.anySelected()) {
      drawTextureCentered(getTexture("lotus"), {cr * 62.5f, titleButtons.getSelectedElement()->position.y}, cubicSize(65.0f + 10.0f * sin(GetTime() * 3.0f)), WHITE, GetTime() * 40.0f);
   }
}
